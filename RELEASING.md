...
3. Publish the notarized GitHub release asset (this defaults to the public
   notarization path and uses the matching changelog section as release notes):
...
After a GitHub release exists and the download URL is public:

```bash
MJUKIS_DEV_PATH=/path/to/mjukis.dev ./scripts/publish-website-release.sh
```
